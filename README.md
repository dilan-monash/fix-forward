...
The approved source combines the existing adult household guide at `/` with Quest, a learning game designed for ages 7–12, at `/quest`. The parent guide at `/quest?view=parents` explains the purpose, learning activities and ways to join in, with familiar Pip/Flo illustrations and FAQs. For current behavior and evidence, start with the [child usability review](docs/QUEST_CHILD_AUDIT_2026-09-13.md) and [graphical game-feedback release notes](docs/QUEST_GAME_FEEL_2026-09-14.md). The [developer handover](docs/DEVELOPER_HANDOVER.md) explains the earlier architecture; later source comments and these release notes supersede its older descriptions of movement settings, audio defaults and local-only status.

**Graphical learning and touch play:** Eight illustrated story islands show validated completion stamps. Picture-based mission steps, scene-linked clues and word help support reading. Children can drag the item's picture or use the tap and Move alternatives. Checked choices receive a tick or cross with an explanation; mixed plans preserve correct choices for retry. The sorting card and its teaching words stay in place while the picture arrives or springs back. Sparks, levels and stars flying into the top bar show progress without awarding the same achievement twice. Hints and retries keep the same available rewards; Sparks are game progress, not a learning score.

**Movement, sound and story voice:** Animations stay on, with bounded touch and reward effects; warning pictures and their words remain still. New adventures enable game sounds, automatic narration and supported touch haptics by default, while explicit saved sound and narration choices remain respected. Browser gesture requirements still apply. Wins use musical feedback rather than automatic congratulatory speech. Matching story MP3s load only when requested; the device voice handles unmatched text when available. Pause, Resume, Stop and visible story words give control over narration. See the [audio notes and licence](quest/audio/README.md) and commented [generation script](scripts/generate-story-audio.mjs).

**Verification boundaries:** The linked release notes record the approved I2 source's automated and responsive-browser checks. Main promotion requires its own checks, exact deployed-commit verification and live route review. Sound and vibration API tests do not establish audible quality or physical-tablet feel. Child comprehension, enjoyment and learning outcomes remain human-review questions. This promotion changes application code without importing or modifying shared Neon data.

For a local preview, run this command from the intended checkout:

```powershell
# The helper defaults to port 5502. Set PORT only when another preview already uses it.
npm.cmd run dev:quest
```

Open **http://127.0.0.1:5502/quest**, **http://127.0.0.1:5502/quest?view=parents** and **http://127.0.0.1:5502/**. The helper uses authored Quest stories and clearly labelled synthetic adult records, with no database connection. It is not the production server. Each port has separate browser progress. See the [earlier implementation guide](docs/QUEST_IMPLEMENTATION.md) and [source register](docs/QUEST_CONTENT_SOURCES.md). Historical notes below describe earlier snapshots, not current release verification.

**Earlier adult usability review, 12 September 2026:** The [mentor-feedback response](docs/MENTOR_FEEDBACK_2026-09-12.md) records the compact landing page, searchable optional brand/model suggestions, clearer warning actions, safety counter placement and bounded recovery from temporary data failures. That dated review is distinct from later unpublished adult-audit work, which is not included in this approved source.
