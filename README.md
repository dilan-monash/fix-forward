...
**Local story, touch and family review, 13 September 2026:** The current uncommitted Iteration 2 work keeps the persistent Sparks bar, earned-star flight and optional game sounds, and adds clearer play around them. Numbered markers on the current scene connect to an enlarged clue picture and its exact explanation. Picture help opens beside the game instead of in a text-only dialog. Children can drag the sorting item's picture directly, or tap the picture and then its destination; the existing Move alternative remains. Two-item answers receive separate ticks/retry explanations, and retry preserves both choices while marking an unchanged correct choice to keep. Reward previews show the actual Sparks still available, and varied celebrations describe the actual award without promising replay points twice.

The parent guide again opens with **For parents and curious families**, familiar Pip/Flo art on a dimensional paper stage, and a **Look → Choose → Discover** example. The age-question panel is removed. The preferred “Throw it away” purpose, four learning aims, “Let your child lead”, fictional preview and FAQs remain. The example includes completed sharing checks and a person who wants the fan; working alone is not presented as enough to give an appliance away.

**Story voice:** Short conversational invitations and authored story lines now have packaged Kokoro AI narration. The browser plays a matching MP3 on request; it does not download a speech model or send child information for generation. Pause/Resume/Stop and visible transcripts cover playback. Unmatched or changed words use the device voice when available; exact-text matching prevents an edited safety fact from playing an old recording. Game sound effects stay separate and opt-in. The [audio notes and licence](quest/audio/README.md) and commented [generation script](scripts/generate-story-audio.mjs) explain how to maintain the recordings.

**Current local checks, 13 September 2026:** `npm.cmd run check` passes all 301 JavaScript checks. The 9 Quest Flask route tests pass, including protected MP3 access, byte ranges and HEAD responses. In Chrome, the current parent introduction and enlarged clue art were reviewed; portrait and landscape tablet previews had no horizontal overflow, with the clue's next action in view. A mixed two-item plan retained its good answer and awarded the advertised 30 Sparks after correction. Dragging the jug artwork directly to the wrong place showed a cross; retrying with picture-tap then destination-tap earned the expected 5 Sparks. Recorded story and character speech showed Play, Pause, Resume, matching visible words and Stop; the browser console had no errors or warnings in these journeys.

This update remains uncommitted and local only. Main and Iteration 1 have not changed. Automated touch events and browser pointer review do not replace a physical-tablet trial. Audible delivery, pronunciation, child comprehension and learning outcomes still need human review; ages 7–12 remain the design audience, not a measured learning claim.

**Movement choice:** The game starts with **Follow my device**, which respects the device's reduced-motion preference. In the top-bar game settings, **Game animations** explicitly enables the finite effects, even when the device asks for less movement. **Less movement** disables them again; **Follow my device** returns to the device preference. The saved choice changes presentation, never points or answers.

**Earlier FixForward Quest tablet refinement, 13 September 2026:** This isolated branch adds a child adventure for ages 7–12 at `/quest`, alongside the adult `/` journey. Its picture choices, simpler story sentences, four local levels, Sparks, two-picture clue reflections, word help and finite art sequences remain part of the game. Parents have a direct guide at `/quest?view=parents`. Hints and retries earn the same rewards; Sparks are game progress, not a learning score. See [the earlier tablet build, exact rules and verification limits](docs/QUEST_TABLET_PLAY.md); the local review above and inline source comments describe the subsequent game-feedback changes.

For the current local review, run the following from this checkout:

```powershell
# Use the current review port; the helper defaults to 5502 when PORT is unset.
$env:PORT = '5504'
npm.cmd run dev:quest
```

Open **http://127.0.0.1:5504/quest**, **http://127.0.0.1:5504/quest?view=parents** and **http://127.0.0.1:5504/**. The helper uses no database: adult records are explicitly labelled synthetic test data, and Quest uses authored stories. It is a local review helper, not the production server. Each port has separate browser progress. See [the current source walkthrough](docs/DEVELOPER_HANDOVER.md), [exact Windows commands and earlier verification](docs/QUEST_IMPLEMENTATION.md), [earlier creative refresh](docs/QUEST_CREATIVE_REFRESH.md), [mentor demo](docs/QUEST_DEMO.md), [comparison worksheet](docs/QUEST_COMPARISON_WORKSHEET.md) and [source register](docs/QUEST_CONTENT_SOURCES.md). The dated baseline notes below describe earlier snapshots, not validation of this local build or its deployment status.
