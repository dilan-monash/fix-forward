...
**11 September access update:** a server-side shared-password gate now precedes application assets and reference APIs. CSRF-protected login/logout POSTs and a signed four-hour access cookie supplement the GET-only reference endpoints in the older diagram below. The cookie contains no appliance answers or credentials. See [website access architecture](WEBSITE_ACCESS.md) and [executed checks](PASSWORD_BRANCH_VERIFICATION_2026-09-11.md).
