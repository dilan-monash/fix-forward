...
Version: 1.4.0-goal-first prototype
...
| T33 | Identify -> Educate -> Action | v1.4 keeps identify + education/safety, but starts with user intent and reduces the progress model to Your appliance -> Quick safety check -> Your options. | Experimental refinement | Mentor/usability review |
...
| U13 | Manual cost comparison low value | v1.4 shows the planned smart product-resolution/evidence flow and keeps manual comparison only as a fallback. It explicitly refuses AI-invented dollar values until governed price evidence exists. |

## Highest-priority live verification before mentor demo

1. Deploy this candidate to a **non-production** Render preview from `feature/i1-feedback-redesign`.
2. Confirm the goal-first landing/loading state appears immediately on a cold start.
...
6. Verify a burning/smoke/electrical critical result can **never** show a community Repair Cafe or cost comparison.
...
16. Record the **scope decision** for device geolocation and the revised severity model before calling either an accepted Iteration 1 requirement.
